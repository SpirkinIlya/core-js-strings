...
function removeLastOccurrences(str, value) {
  if (str.lastIndexOf(value) !== -1) {
    const beginningOfString = str.slice(0, str.lastIndexOf(value));
    const endigOfString = str.slice(str.lastIndexOf(value) + value.length);
    return beginningOfString + endigOfString;
  }
  return str;
}
...
function isPalindrome(str) {
  const strWithoutSpaces = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  const reversedString = strWithoutSpaces.split('').reverse().join('');
  if (reversedString === strWithoutSpaces) {
    return true;
  }
  return false;
}
...
function extractNameFromTemplate(value) {
  const valueNew = value.replace('!', '');
  const words = valueNew.split(' ');
...
function getCardId(value) {
  ...
  return deckArray.indexOf(value);
}
